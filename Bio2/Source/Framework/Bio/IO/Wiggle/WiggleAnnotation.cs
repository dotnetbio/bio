...
namespace Bio.IO.Wiggle
{
    ...
    public class WiggleAnnotation : IEnumerable<KeyValuePair<long, float>>
    {
        ...
        private List<string> annotationComments = new List<string>();
        ...
        public WiggleAnnotation(float[] data, string chromosome, long start, int step, int span)
        {
            ...
            this.Metadata.Add(WiggleSchema.Type, WiggleSchema.Wiggle0);
        }
        ...
        public KeyValuePair<long, float> this[long index]
        {
            get
            {
                ...
                {
                    return new KeyValuePair<long, float>(this.BasePosition + (index * this.Step), this.fixedStepValues[index]);
                }
                else
                {
        ...
        public float[] GetValueArray(long startIndex, long length)
        {
            ...
            {
                throw new NotSupportedException(Properties.Resource.WiggleNotSupportedOnVariableStep);
            }

            float[] result = new float[length];
            Array.Copy(this.fixedStepValues, startIndex, result, 0, length);
