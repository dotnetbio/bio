...
namespace Bio.Util
{
	...
	public static class Helper
	{
		...
		public static SequenceStrandType GetStrandType(string strand)
		{
			...
			strand = strand.ToLowerInvariant();
		...
		public static SequenceStrandTopology GetStrandTopology(string strandTopology)
		{
			...
			strandTopology = strandTopology.ToLowerInvariant();
		...
		public static void CheckCondition<T>(bool condition) where T : Exception, new()
		{
		...
		/// <param name="message">A message for the exception</param>
		/// <typeparam name="T">The type of exception that will be raised.</typeparam>
		public static void CheckCondition<T>(bool condition, string message) where T : Exception, new()
		{
			if (!condition)
			{
				throw new T();
		...
		/// <param name="formatValues">Values for the exception's message.</param>
		/// <typeparam name="T">The type of exception that will be raised.</typeparam>
		public static void CheckCondition<T>(bool condition, string messageToFormat, params object[] formatValues) where T : Exception, new()
		{
			if (!condition)
			{
		...
		/// <param name="condition">The condition to check</param>
		/// <param name="messageFunction">Function that will generate the message if the condition is false.</param>
		public static void CheckCondition<T>(bool condition, Func<string> messageFunction) where T : Exception, new()
		{
			if (!condition)
			{
				string message = messageFunction();
		...
		public static bool KeysEqual<TKey, TValue1, TValue2>(IDictionary<TKey, TValue1> dictionary1, IDictionary<TKey, TValue2> dictionary2)
		{
			return dictionary1.Count == dictionary2.Count 
				&& dictionary1.Keys.All(dictionary2.ContainsKey);
		}
		...
		public static bool ASCIIEncoding(byte[] input, out string output)
		{
			output = null;
			if (input == null) 
				return false;
		...
		{
			string badSequence;
			ASCIIEncoding(data, out badSequence);
		...
		{
			ArrayExtensions.LongCopy(sourceArray, sourceIndex, destinationArray, destinationIndex, length);
		}
	}
}
