...
namespace Bio.Algorithms.Alignment
{
    ...
    public class ClusterBuilder : IClusterBuilder
    {
            ...
            for (counter1 = 0; counter1 < matches.Count ; counter1++)
            {
                        ...
                        counter2++)
                {
                    ...
                    if (matches[counter1].QuerySequenceOffset <= matches[counter2].QuerySequenceOffset)
                    {
                        ...
                        {
                            long overlap;
                            if (matches[counter1].ReferenceSequenceOffset == matches[counter2].ReferenceSequenceOffset)
                            {
        ...
        private static List<MatchExtension> Sort(IEnumerable<MatchExtension> matches, string sortBy)
        {
