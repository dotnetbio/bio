...
namespace Bio.IO.SAM
{
    ...
    public class SAMOptionalField
    {
        ...
        /// </summary>
        private static readonly char[] VTypeAllowableValues = "AifZHB".ToCharArray();

        /// <summary>
        /// Holds illegal characters for value.
        /// </summary>
        private static readonly char[] ValueIllegalCharacters = {'\t','\n','\r'};

        /// <summary>
        /// Holds regular expression for Tag.
        /// </summary>
        private static readonly Regex TagRegexExpr = new Regex(TagRegexExprPattern);
        ...
        {
            return string.IsNullOrEmpty(tag) || tag.Length != 2 || !ValidateTagRegex(tag)
                       ? string.Format(Properties.Resource.InvalidPatternMessage, "Tag", tag, TagRegexExpr)
                       : string.Empty;
        }
