namespace SequenceAssembler
{
    ...
    public class Controller : IDisposable
    {
        ...
        /// </summary>
        // TODO:
        //private IBlastWebHandler lastExecutedBlastService;
        ...
        */
        ...
        private static void CreateImportedSequence(string filePath, Collection<ParsedFileInfo> importedSequence, ISequenceParser parser = null)
        {
            ...
            IEnumerable<ISequence> sequenceList = parser.Parse(filePath);
        ...
        private static void ParseAllFormats(string filePath, Collection<ParsedFileInfo> importedSequence, BackgroundWorker worker)
        {
            foreach (ISequenceParser parser in SequenceParsers.All)
            {
                if (worker != null && !worker.CancellationPending)
                {
                    ...
                    {
                        if (SequenceParsers.All.Any(p => p == parser))
                        {
                            // Found a parser.
                            continue;
                        }
        ...
        private static AssemblyOutput RunAssembly(AssemblyInputEventArgs input, BackgroundWorker worker)
        {
            ...
            if (input.AssemblerUsed == AssemblerType.PaDeNA)
            {
                ...
                {
                    KmerLength = input.KmerLength,
                    DanglingLinksThreshold = input.DanglingLinksThreshold,
                    RedundantPathLengthThreshold = input.RedundantPathLengthThreshold,
                    AllowErosion = input.ErosionEnabled
                };
            ...
            else
            {
                ...
                {
                    OverlapAlgorithm = ChooseAlgorithm(input.Algorithm),
                    MergeThreshold = mergeThreshold,
                    ConsensusResolver = new SimpleConsensusResolver(input.ConsensusThreshold),
                ...
                if (input.AlignerInput.SimilarityMatrix == null)
                {
        ...
        private static IBlastWebHandler GetWebServiceInstance()
        {
            return new NcbiBlastWebHandler();
        }
        */
        ...
        private void OnExecuteSearch(object sender, WebServiceInputEventArgs e)
        {
            ...
            {
                var blastServiceHandler = new NcbiBlastWebHandler();
        ...
        private void OnBlastRequestCompleted(object sender, Exception ex, Stream stm)
        {
            RunWorkerCompletedEventArgs ev = null;
            if (ex == null)
            {
                ev = new RunWorkerCompletedEventArgs(
                        new StreamReader(stm).ReadToEnd(),
                        null, false);
            }
            else
            {
