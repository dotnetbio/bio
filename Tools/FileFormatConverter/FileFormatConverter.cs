...
namespace FileFormatConverter
{
    public class FileFormatConverter
    {
        ...
        public void ConvertFile()
        {
            ...
            var inputParser = SequenceParsers.FindParserByFileName(this.InputFile);
            if (inputParser == null)
                throw new Exception("Input file not a valid file format to parse.");

            //Finds a formatter and opens the file
            var outputFormatter = SequenceFormatters.FindFormatterByFileName(this.OutputFile);
            if (outputFormatter == null)
                throw new Exception("Output file not a valid file format for conversion.");

            using (inputParser.Open(this.InputFile))
            using (outputFormatter.Open(this.OutputFile))
            {
                try
                {
                    foreach (var sequence in inputParser.Parse())
