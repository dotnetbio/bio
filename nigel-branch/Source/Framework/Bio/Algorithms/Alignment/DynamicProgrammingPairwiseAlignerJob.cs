...
namespace Bio.Algorithms.Alignment
{
    ...
    public abstract class DynamicProgrammingPairwiseAlignerJob
    {
        ...
        {
            //Fill Column
            for (long k = 0, cellIndex = Math.BigMul(blockRow, gridStride), foldIndexx = gridStride; k <= lastRow; k++, cellIndex++, foldIndexx--)
            {
                fold[foldIndexx] = cachedCols[blockCol][cellIndex];
            }

            //Fill Row
            for (long k = 0, cellIndex = Math.BigMul(blockCol, gridStride), foldIndexx = gridStride; k <= lastCol; k++, cellIndex++, foldIndexx++)
            {
        ...
        protected void WritebackFold(int[][] cachedRows, int[][] cachedCols, int[] fold, int blockRow, int blockCol, int lastRow, int lastCol)
        {
            ...
            {
                for (long k = 0, cellIndex = Math.BigMul(blockCol, gridStride), foldIndexx = 0; k <= lastCol; k++, cellIndex++, foldIndexx++)
                {
            ...
            {
                for (long k = 0, cellIndex = Math.BigMul(blockRow, gridStride), foldIndexx = 2 * gridStride; k <= lastRow; k++, cellIndex++, foldIndexx--)
                {
